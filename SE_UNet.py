...
class CATConv(nn.Module):
    def __init__(self, in_channel=1, out_channel1=1): 
        self.in_channel = in_channel
        self.out_channel = out_channel1
        super(CATConv, self).__init__()
        self.conv1 = nn.Conv3d(in_channel, out_channel1, kernel_size=1, stride=1, padding=0, bias=False)
        self.norm = nn.InstanceNorm3d(out_channel1)
        self.act = nn.LeakyReLU(inplace = True)
    def forward(self, x):
        e0 = self.conv1(x)
...
class DMR_UNet(nn.Module):
    def __init__(self, in_channel=1, n_classes=1):
        ...
        self.ec33 = CATConv(56, 32)
        self.x33 = CATConv(self.in_channel, 32)
        ...
        self.ec63 = CATConv(128, 64)
        self.x63 = CATConv(self.in_channel, 64)
        ...
        self.ec93 = CATConv(192, 64)
        self.x93 = CATConv(self.in_channel, 64)
    ...
    def forward(self, x): #1 2 128 128 128
        ...
        e1, s1 = self.ec2(e0)
        e1_1, s2 = self.ec3(e1) 
        e1=self.ec33(torch.cat((e1_1,e0,e1),1))
        e1=e1+self.x33(x)
        e2 = self.pool0(e1)
        x=self.pool0(x)
        # print(e2.shape)
        ...
        e3, s4 = self.ec5(e2)
        e3_1, s5 = self.ec6(e3)  
        e3=self.ec63(torch.cat((e3_1,e2,e3),1))
        e3=e3+self.x63(x)
        e4 = self.pool1(e3)
        x= self.pool1(x)
        # print(e4.shape)
        ...
        e5, s7 = self.ec8(e4)
        e5_1, s8 = self.ec9(e5)
        e5=self.ec93(torch.cat((e5_1,e4,e5),1))
        e5=e5+self.x93(x)
        e6 = self.pool2(e5)
        # print(e6.shape)
        ...
        e6, s9 = self.ec10(e6)
        e7, s10 = self.ec11(e6) 
        e7_1, s11 = self.ec12(e7) 
        e7=self.ec123(torch.cat((e7_1,e6,e7),1))
        
        e8 = self.up_sample0(e7)

        d0, s12 = self.dc1(torch.cat((e8,e5),1))
        d0_1, s13 = self.dc2(d0)
        d0=self.dc22(torch.cat((d0_1,d0),1))
        
        d1 = self.up_sample1(d0)

        d1, s14 = self.dc3(torch.cat((d1,e3),1))
        d1_1, s15 = self.dc4(d1)
        d1=self.dc42(torch.cat((d1_1,d1),1))
        
        d2 = self.up_sample2(d1)

        d2, s16 = self.dc5(torch.cat((d2,e1),1))
        d2_1, s17 = self.dc6(d2)
        d2=self.dc62(torch.cat((d2_1,d2),1))
        ...
        # return pred0, pred1,want1,want2,want3,want4,want5,want6
        return pred0, pred1
...
if __name__ == '__main__':
    ...
    if use_gpu:
        net = net.cuda()
        inputs = torch.randn(1,2,128,128,128).cuda()
    else:
